...
// #endif  //__GNUC__
/*
	Command_Packet represents the 12 uint8_t command that we send to the finger print scanner
*/
class Command_Packet
{
	...
	private: 
		static const uint8_t COMMAND_START_CODE_1 = 0x55;	// Static uint8_t to mark the beginning of a command packet	-	never changes
		static const uint8_t COMMAND_START_CODE_2 = 0xAA;	// Static uint8_t to mark the beginning of a command packet	-	never changes
		static const uint8_t COMMAND_DEVICE_ID_1 = 0x01;	// Device ID Byte 1 (lesser uint8_t)							-	theoretically never changes
		static const uint8_t COMMAND_DEVICE_ID_2 = 0x00;	// Device ID Byte 2 (greater uint8_t)							-	theoretically never changes
		uint8_t command[2];								// Command 2 bytes

		uint8_t _CalculateChecksum();						// Checksum is calculated using uint8_t addition
		uint8_t GetHighByte(uint8_t w);						
		uint8_t GetLowByte(uint8_t w);
};
// #ifndef __GNUC__
// #pragma endregion
// #endif  //__GNUC__
	...
	public:
		class ErrorCodes
		{
			public:
				...
				static Errors_Enum ParseFromBytes(uint8_t high, uint8_t low);
		};
...
// #endif  //__GNUC__
// Data Mule packet for receiving large data(in 128 uint8_t pieces) from the FPS
...
// #endif  //__GNUC__
	...
	// Not implemented due to memory restrictions on the arduino
	// may revisit this if I find a need for it
	//int SetTemplate(uint8_t* tmplt, int id, bool duplicateCheck);
...
// #endif  //__GNUC__
	...
	void SendToSerial(uint8_t data[], int length);
...
private:
	 void SendCommand(uint8_t cmd[], int length);
